...
-module(machi_flu_wedge_manager).

-behaviour(gen_server).

-type epoch_id() :: undefined | { Id :: non_neg_integer(), Csum :: binary() }.

-record(state, {
          cmgr_pid :: pid(), %% chain manager pid
          wedged = true :: boolean(),
          etstab :: ets:tid(),
          epoch_id :: epoch_id()
}).
...
start_link([FluName, ChainMgrPid]) ->
    gen_server:start_link({local, ?MODULE}, ?MODULE, [FluName, ChainMgrPid], []).
...
%% gen_server callbacks
init([FluName, ChainMgrPid]) ->
    Tid = ets:new(make_name(FluName, "_epoch"), [set, protected, named_table, {read_concurrency, true}]),
    true = ets:insert(Tid, {epoch, {true, 0}}),
    {ok, #state{epoch_id=0, etstab=Tid, cmgr_pid=ChainMgrPid}}.
...
handle_info({wedge_myself, _EpochId}, S = #state{wedged = true}) ->
    ...
    {noreply, S};
handle_info({wedge_myself, EpochId}, S = #state{cmgr_pid = CMPid,
                                                wedged = false,
                                                epoch_id = E, 
                                                etstab = Tid}) when EpochId == E ->
    true = ets:insert(Tid, {epoch, {true, E}}),
    kick_chain_manager(CMPid),
